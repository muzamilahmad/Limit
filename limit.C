...
//**************************************************Theory predicted
TGraph *cgr1=new TGraph(n,x,cxnl);
TGraph *cgr2=new TGraph(n,x,cxnu);
   TGraph *cgrshade = new TGraph(2*n);
for (int i=0;i<n;i++) {

      cgrshade->SetPoint(i,x[i],cxnu[i]);
      cgrshade->SetPoint(n+i,x[n-i-1],cxnl[n-i-1]);
   }



   cgrshade->SetFillStyle(3001);
   cgrshade->SetFillColor(kBlue);
   cgrshade->Draw("f");
   cgr1->Draw("l");
   cgr2->Draw("l");
   grT->SetLineWidth(2);
